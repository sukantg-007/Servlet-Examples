...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	<%!
	String path="D:\\Student\\Sanket\\servlet\\webapp06\\WebContent\\cakeimage"+File.separator;
			...
			<div class="row m-5">
				<div class="col mt-5">
					<div class="card" style="min-height: 40rem;min-width:1200px;">
						<div class="card-body">
							<h2 class="card-title text-center">Cake Data</h2>
							<table class="table table-bordered border-primary table-hover">
								...
								<tbody>
									<%										
										List<Cake> list = service.getAllCake();									
										Iterator<Cake> it = null;
										if (list != null) {
											it = list.iterator();
											while (it.hasNext()) {
												Cake cake = it.next();
												int cid=cake.getCid();
												String imgPath=cake.getCimgPath().substring(path.length());
									%>
									<tr>
										<td><%=cid%></td>
										<td><img class="img-thumbnail" src="./cakeimage/<%=imgPath%>"
											width="100" height="90"></td>
										<td><%=cake.getCname()%></td>
										<td><%=cake.getPrise()%></td>
										
										<td><a class='btn' href='./update?id="+cid+"'>Update</a></td>
										<td><a class='btn' href='./delete?id="+cid+"'>Delete</a></td>
									</tr>
									<%
										}
										} else {
		...
		<nav aria-label="...">
			...
			<%
				...
				for(int i=1;i<=showPages;i++){
					%>
					<li class="page-item"><a class="page-link" href="showCake.do?page=<%=i%>"><%=i%></a></li>
					<%
				}
			%>			
